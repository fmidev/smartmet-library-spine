...
void prelude(SmartMet::Spine::Reactor& reactor, const std::string& handler_path)
{
  try
  {
    ...
    cout << endl << "Testing " << handlers[handler_path] << "  plugin"
         << std::endl << "============================"
         << std::endl;
  }
  catch (...)
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    if (opt.count(nm_config)) {
        ...
        throw Fmi::Exception::Trace(BCP, "Mandatory command line option --reactor-config (or -c) missing");
    }
    options.quiet = true;
    ...
    try
    {
        ...
        if (opt.count(nm_handler)) {
            ...
            prelude_funct = [handler_path](SmartMet::Spine::Reactor& reactor) { prelude(reactor, handler_path); };
        } else {
