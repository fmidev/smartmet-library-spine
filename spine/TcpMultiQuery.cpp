    ...
    const std::string& request_body)
{
    ...
    if (query_map.emplace(id, item).second) {
        ...
        item->resolver
            .async_resolve(
                host, service,
                std::bind(&Query::on_endpoint_resolved, &*item, _1, _2)
                );
    } else {
...
TcpMultiQuery::Response
TcpMultiQuery::operator[] (const std::string& id) const
{
...
void
TcpMultiQuery::Query::on_endpoint_resolved(
    boost::system::error_code error,
    boost::asio::ip::tcp::resolver::results_type result)
{
    if (error) {
        ...
        async_connect(
            socket,
            result,
            std::bind(&Query::on_connected, this, _1));
    }
}

void
TcpMultiQuery::Query::on_connected(
    boost::system::error_code error)
{
    if (error) {
...
void
TcpMultiQuery::Query::on_request_sent(
    boost::system::error_code error,
    std::size_t bytes_transfered)
{
...
void
TcpMultiQuery::Query::on_data_received(
    boost::system::error_code error,
    std::size_t bytes_transfered)
{
...
void
TcpMultiQuery::Query::request_input()
{
    using namespace std::placeholders;
    socket.async_read_some(
        boost::asio::buffer(current_input, Query::buffer_size),
        boost::bind(
            &Query::on_data_received,
            this,
...
void
TcpMultiQuery::Query::report_error(
    const std::string& desc,
    boost::system::error_code error)
{
